...
struct GraphEdge {
  GraphEdge(uint64 argv0, uint64 argv1) : v0(argv0), v1(argv1) {}
  uint64 v0;
  uint64 v1;
};
...
void make_graph(graph* g, int num_nodes, const vector<GraphEdge>& edges) {

  g->num_edges = edges.size();
  g->num_nodes = num_nodes;
  
  g->offsets = new int[g->num_nodes];
  g->edges = new Vertex[g->num_edges];
...
void resort_graph_nodes(graph* g, int* node_groups) {
  ...
  */
  ...
  for (int i=0; i<g->num_nodes; i++) {
    int start_edge = g->offsets[i];
    int end_edge = (i == g->num_nodes-1) ? g->num_edges : g->offsets[i+1];
    int newNodeId = oldToNew[i];
    for (int e=start_edge; e<end_edge; e++)
      edges.push_back(GraphEdge(newNodeId, oldToNew[g->edges[e]]));
  }
...
void sanity_check(uint64 num_verts, const vector<GraphEdge>& edges) {
...
int main(int argc, char** argv) {
  ...
  while (getline(file, line)) {
    ...
    edges.push_back(GraphEdge(v0, v1));
